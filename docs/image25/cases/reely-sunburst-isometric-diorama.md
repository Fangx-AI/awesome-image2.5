# 等距建筑剖面

[来源索引](../README.md) · [31 类创作图谱](../../../skills/image25/references/gallery.md) · [如何使用提示词](../../prompts.md)

作者：ReelyArt · 分类：建筑与场景

模型依据：GPT Image 2.5 Sunburst · 平台声明

原页提供示例图；未提供与该图逐项配对的完整生成记录。

![等距建筑剖面 · 1](https://cdn.reely.art/models/gpt-image-2-5/sunburst-isometric-diorama.webp)

## 提示词与复现

[查看原页及已公开的提示词资料](https://reely.art/models/gpt-image-2-5-sunburst)

原页未公开完整提示词或参数时，不补造“原始 Prompt”。参考图编辑需要作者公开的输入图，或使用你自己的参考图。

## 来源记录

[原始出处](https://reely.art/models/gpt-image-2-5-sunburst)

Original publisher; external image reference

[继续浏览来源作品](../README.md) · [反馈图片或来源错误](https://github.com/Fangx-AI/awesome-image2.5/issues/new?template=correction.md)
